...
Push-Benachrichtigungen sind kein offener Punkt mehr: sie sind gebaut und scharfgeschaltet,
siehe `benachrichtigungen-plan.md`.
...
**Seit dem 16./17.09.2026 gibt es dazu ein ausgearbeitetes Konzept** (Entscheidungsgrundlage,
noch nichts davon gebaut): eine Faustregel, was offline überhaupt geschrieben werden darf
(„eine Person, ein Ort, ein Zeitpunkt, rückgängig machbar" – z. B. Auftragsstatus,
Technikernotiz, Radmessung), eine Warteschlange aus **Absichten** statt fertiger Datenzeilen
(„setze Status auf erledigt", nicht „Auftrag = {…}"), eine ehrliche Markierung noch nicht
übertragener Änderungen in der Oberfläche, und eine dreistufige Konfliktbehandlung
(unverändert → übernehmen, anderes Feld geändert → zusammenführen, gleiches Feld geändert →
nachfragen). Empfohlen wird darin ein kleiner Zuschnitt (nur Status, Notiz, Radmessung) statt
des großen (alles am Auftrag außer Abschließen) – Näheres dazu außerhalb dieses Repositoriums
in der Projektablage. **Das ändert nichts am Ist-Zustand hier: Es ist weiterhin nichts
gebaut, ohne Netz lässt sich in keinem Modul etwas schreiben.**

### Push-Benachrichtigungen — UMGESETZT (Terminerinnerung, siehe `benachrichtigungen-plan.md`)

Stand bis 09.09.2026 hier als „Optional" und noch ohne Anlass geführt. Inzwischen gebaut und
scharfgeschaltet: Fünf Minuten vor einem Termin bekommt der zugeordnete Techniker eine
Push-Benachrichtigung, die direkt ins Auftragsfenster führt (`app/api/push/*`,
`components/PushEinstellung.tsx`, `public/sw.js`). Die Voraussetzung aus diesem Abschnitt
gilt unverändert: **nur** auf iOS ab Version 16.4 und **nur**, wenn die App auf dem
Home-Bildschirm installiert ist – Stufe 1 dieses Dokuments ist damit für Push nicht optional,
sondern Voraussetzung. Details, offene Punkte (u. a. der reale Gerätetest bei gesperrtem
Bildschirm und im Fokus „Fahren") und die Einrichtung stehen in `benachrichtigungen-plan.md`.
...
| `proxy.ts` | Ausnahmen für die PWA-Dateien |
...
## Was seit dem 10.09.2026 dazukam (Ist-Zustand, Härtung von Stufe 2)

Zwei Befunde aus der Browser-Konsole am 14.09.2026, beide ohne sichtbaren Fehler in der
Anwendung selbst – nur an der Typografie bzw. an einer roten Zeile in der Konsole erkennbar:

**1. Die Google-Schriften liefen still auf Ersatzschriften um.** Der Service Worker fängt
Schriftanfragen ab (`SCHRIFT_HOSTS` in `public/sw.js`) und holt sie selbst per `fetch`. Ein
`fetch` AUS dem Worker heraus ist ein Verbindungsaufbau und fällt damit unter die
CSP-Direktive `connect-src` – nicht unter `style-src`/`font-src`, die beide längst offen
...
Antwort. `ausSpeicherOderNetz()` in `public/sw.js` gibt seitdem bei einem gescheiterten Abruf
eine reguläre `504`-Antwort mit dem Grund im Text zurück, statt die Ausnahme durchzureichen –
die Seite lädt dann weiter, nur ohne das eine blockierte Stück, und die Konsole zeigt das
eigentliche Problem statt eines irreführenden „Uncaught (in promise) TypeError".

Beide Änderungen sind reine Härtung von Stufe 2 (Programmhülle) und ändern nichts an ihrem
Umfang: weiterhin liegt kein Kundenname und keine Adresse dauerhaft auf dem Gerät, weiterhin
gehen Supabase, Kartenkacheln und `/api/`-Aufrufe unberührt ans Netz.
