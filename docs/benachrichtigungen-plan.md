...
„Einrichtung der Terminerinnerung" sind erledigt, die Terminerinnerung verschickt seither
tatsächlich Meldungen, nicht nur die Testnachricht. **Offen bleibt weiterhin nur ein Punkt:**
der reale Gerätetest bei gesperrtem Bildschirm und im Fokus „Fahren" auf einem echten
Diensthandy – siehe „Vortest" am Ende. Der Vortest („Testnachricht an mich") hat gezeigt, dass
Meldungen auf dem iPhone grundsätzlich ankommen; das ist etwas anderes als der Test unter den
Bedingungen, für die die Funktion gebaut wurde.
...
**5. Der Sprung ins Auftragsfenster (gebaut).** Die Nachricht trägt eine Zieladresse mit,
`/?auftrag=‹id›` (`AUFTRAG_PARAMETER` in `lib/constants.ts`) – ursprünglich als
`/?kunde=‹id›` geplant, siehe die Korrektur weiter unten. Der Service Worker fängt das
Antippen ab (`notificationclick`), holt ein bereits offenes Fenster nach vorn oder öffnet ein
neues. Die Anwendung liest den Parameter beim Start und öffnet das Auftragsfenster – nach dem
Muster, das für den QR-Aufkleber am Lagerregal schon existiert (`?lagerplatz=` in
`app/page.tsx`), inklusive des Bereinigens der Adresszeile danach.

## Einrichtung der Terminerinnerung — ERLEDIGT (Stand 18.09.2026)

Der Code war zunächst da, ohne dass die Erinnerung tatsächlich verschickte; scharf wurde sie
erst durch drei Handgriffe, die inzwischen alle vollzogen sind:

1. ~~**Migration 27** (`push_versand`) im SQL-Editor ausführen~~ – die Doppelmeldungssperre.
   Erledigt.
2. ~~**Ein Geheimnis erzeugen** und an zwei Stellen hinterlegen.~~ Erledigt. Es schützt die
   Versandroute: ohne dieses Kopffeld weist sie jeden Aufruf ab, sonst könnte jeder im
   Internet Meldungen auf fremden Sperrbildschirmen auslösen.
   - bei Vercel als Umgebungsvariable `PUSH_GEHEIMNIS`, danach einmal neu deployt
...
3. ~~**Migration 28** ausführen~~ – schaltet `pg_cron`/`pg_net` ein und legt den Minutentakt an.
   Erledigt; der Zeitgeber läuft seither im Minutentakt.

Der reale Betrieb bestätigt das: `tests/terminerinnerung.test.ts` deckt die Fensterlogik
(„genau fünf Minuten vorher, Nachholfenster bis zum Termin, danach Schweigen") ab, und die
Versandroute (`app/api/push/senden`) trägt seither tatsächlich Meldungen ein und verschickt
sie – nicht nur im Test.

Falls doch einmal nichts ankommt, lässt sich das ohne Warten auf einen echten Termin prüfen:
einen Auftrag auf heute mit einer Uhrzeit sechs Minuten in der Zukunft anlegen, sich selbst als
Mitarbeiter zuordnen (das eigene Benutzerkonto muss im Admin-Bereich mit diesem Mitarbeiter
verknüpft sein) und das Handy weglegen. Kommt nichts, in dieser Reihenfolge nachsehen:
...
Deshalb steht `api/push/senden` seit dem in der Ausnahmeliste des Matchers in `proxy.ts`.
...
**Empfehlung: `pg_cron` plus `pg_net`.** Der Auftrag steht dann als Migration in `supabase/migrations/` wie
alles andere an der Datenbank, läuft im Minutentakt und ruft die
...
1. **Voller Text**: „Termin in 5 Min: Max Mustermann, Musterstraße 1". Am schnellsten
   erfassbar, zeigt aber Name und Adresse offen an.
2. **Nur der Name**: „Termin in 5 Min: Max Mustermann". Die Adresse steht nach dem Antippen.
...
* `?auftrag=`-Parameter in `app/page.tsx` nach dem Muster von `?lagerplatz=`
...
**Stand 18.09.2026: Der Vortest lief nur mit der einfachen Testnachricht bei entsperrtem
Bildschirm (siehe unten) – auf dieser Grundlage wurde der Rest (Zeitgeber, Versand,
Terminerinnerung) trotzdem gebaut und scharfgeschaltet.** Die eigentliche Frage aus diesem
Abschnitt – gesperrter Bildschirm, Fokus „Fahren", auf einem echten Diensthandy – ist damit
**nicht** rückwirkend beantwortet und bleibt der eine offene Punkt dieses Dokuments.
