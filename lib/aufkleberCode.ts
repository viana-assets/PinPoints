// Was auf einem QR-Aufkleber steht – Erzeugen und Zurücklesen an EINER Stelle.
//
...
// ---------------------------------------------------------------- Reifensatz (17.09.2026)
//
// Das Etikett am Reifensatz beantwortet eine andere Frage als das am Regal: nicht „welcher Platz
// ist das", sondern „wem gehört der hier, der falsch steht". Es klebt am Satz und wandert mit
// ihm; wo er liegt, steht deshalb NICHT als Kennung darauf, sondern wird beim Scannen
// nachgeschlagen. Ein Aufkleber, der einen Lagerplatz behauptet, wäre in dem Moment falsch, in
// dem jemand den Satz umräumt – und genau dann wird er gebraucht.
export function satzUrl(einlagerungId: string, basis: string): string {
  const ursprung = basis.replace(/\/+$/, "");
  return `${ursprung}/?${SATZ_PARAMETER}=${einlagerungId}`;
}

export function satzIdAusCode(text: string): string | null {
  const roh = (text || "").trim();
  if (!roh) return null;
  try {
    const url = new URL(roh);
    const wert = url.searchParams.get(SATZ_PARAMETER);
    if (wert && UUID_MUSTER.test(wert)) return wert.toLowerCase();
    return null;
  } catch {
